...
Contacts* ContactsInit()
{
	...
	p->next = cur;
	Contacts* curnext=cur;
	while (fread(curnext, sizeof(Contacts), 1, doc))
	{
		curnext = (Contacts*)malloc(sizeof(Contacts));
		curnext->next = NULL;
		cur->next = curnext;
		cur = cur->next;
	}
...
void ContactsChange(Contacts* p)
{
	...
	printf("请输入需要改变联系人的性别；");
	scanf("%s", cur->human.sex);
	printf("请输入需要改变联系人的年龄；");
	scanf("%d", &(cur->human.age));
	printf("请输入需要改变联系人的电话号码；");
